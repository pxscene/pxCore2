...
then
  ...
  if [ "$TRAVIS_EVENT_TYPE" != "cron" ] && [ "$TRAVIS_EVENT_TYPE" != "api" ] ;
  then
    cmake -DBUILD_PX_TESTS=ON -DBUILD_WITH_PXOBJECT_TRACKING=ON  -DBUILD_PXSCENE_STATIC_LIB=ON -DBUILD_DEBUG_METRICS=ON .. >>$BUILDLOGS 2>&1;
  else
    cmake .. >>$BUILDLOGS 2>&1;
  fi
  ...
  cmake -DBUILD_WITH_PXOBJECT_TRACKING=ON  -DBUILD_PX_TESTS=ON -DBUILD_PXSCENE_STATIC_LIB=ON -DBUILD_DEBUG_METRICS=OFF .. 1>>$BUILDLOGS;
