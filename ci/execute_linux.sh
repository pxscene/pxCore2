...
./spark.sh -disableFilePermissionCheck=true $TESTRUNNERURL?tests=$TESTS > $EXECLOGS 2>&1 &
...
#check for crash
$TRAVIS_BUILD_DIR/ci/check_dump_cores_linux.sh `pwd` Spark $ACCESSFAILLOGS
retVal=$?
if [ "$retVal" -eq 1 ]
        then
        if [ "$TRAVIS_PULL_REQUEST" != "false" ]
...
if [ "$testRunnerRetVal" -ne 0 ]
        ...
        checkError $testRunnerRetVal "Testrunner failure for access test" "$errCause" "Follow the steps locally: Run ./spark.sh https://px-apps.sys.comcast.net/pxscene-samples/examples/px-reference/test-run/testRunner_AccessTest.js locally and check for 'Failures: 0' in logs. Analyze whether failures is present or not"
        exit 1;
fi
