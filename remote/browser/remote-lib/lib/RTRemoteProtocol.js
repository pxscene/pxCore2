...
class RTRemoteProtocol {
  ...
  start() {
    const that = this;
    this.transport.socket.onmessage = (evt) => {
      that.incomeMessage(RTRemoteSerializer.fromString(evt.data));
    };
  }
  ...
  incomeMessage(message) {
    ...
    if (callContext) { // call context
      ...
      this.transport.send(RTRemoteSerializer
        .toString(RTMessageHelper.newKeepAliveResponse(message[RTConst.CORRELATION_KEY], RTStatusCode.OK)));
      ...
      this.transport.send(RTRemoteSerializer
        .toString(RTMessageHelper.newOpenSessionResponse(message[RTConst.CORRELATION_KEY], message[RTConst.OBJECT_ID_KEY])));
    } else {
  ...
  sendCallResponse(correlationKey) {
    this.transport.send(RTRemoteSerializer
      .toString(RTMessageHelper.newCallResponse(correlationKey, null, RTStatusCode.OK)));
  }
  ...
  sendRequestMessage(messageObj) {
    ...
    this.transport.send(RTRemoteSerializer.toString(messageObj));
    return new Promise((resolve, reject) => {
...
/**
 * create new RTRemoteProtocol
 * @param {RTRemoteWBTransport} transport the connection tranport
 * @param {boolean} transportOpened is the transport opened or not
