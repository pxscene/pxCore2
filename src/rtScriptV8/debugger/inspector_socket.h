...
class InspectorSocket {
 public:
  ...
  bool connection_eof;
 private:
  void operator=(const InspectorSocket&) = delete;
  void operator=(InspectorSocket&&) = delete;
  InspectorSocket(const InspectorSocket&) = delete;
  InspectorSocket(InspectorSocket&&) = delete;
};
...
inline InspectorSocket* inspector_from_stream(uv_tcp_t* stream) {
  return (InspectorSocket*)stream->data;
}
