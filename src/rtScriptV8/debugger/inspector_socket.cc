...
static int header_value_cb(http_parser* parser, const char* at, size_t length) {
  ...
  if (state->current_header.size() == sizeof(SEC_WEBSOCKET_KEY_HEADER) - 1 && (0 == strncasecmp(state->current_header.data(),SEC_WEBSOCKET_KEY_HEADER,sizeof(SEC_WEBSOCKET_KEY_HEADER) - 1))){
