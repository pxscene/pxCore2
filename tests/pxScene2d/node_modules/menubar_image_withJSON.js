...
var pxFactory = require("pxFactory");

var factory = new pxFactory(scene);
...
function MenuBarImage(parentScene, name, x, y, w, h, title, imageItems) {
	...
	this.container = factory.create({t:"image",parent:this._scene, a:1, x:x, y:y, w:w, h:h, clip:false});
	...
	this.container.on('onFocus', function(e) {
		...
		this.lineTopFocus = factory.create({t:"image9",url:this.urlFocus,x:image.x,y:image.y-4,w:image.w,h:4,parent:this.container});
		console.log("creating lineBtmFocus in menubar_image_4_usingClass");
		this.lineBtmFocus = factory.create({t:"image9",url:this.urlFocus,x:image.x,y:image.y+image.h,w:image.w,h:4,parent:this.container});
	...
	this.container.on('onKeyDown', function(e) {
	  ...
	  // Handle left/right keys for navigation
          ...
          console.log("focusIndx="+this.focusIndx+" and imageObjs size is "+this.imageObjs.length);
          console.log("this.imageObjs="+this.imageObjs);
...
MenuBarImage.prototype.setMenuItems = function(newText, newImageUrls) {
	console.log("setMenuItems newText is "+newText);
...
MenuBarImage.prototype.showMenu = function() {
	...
	if( this.textTitle1 == null) {
		this.textTitle1 = factory.create({t:"text",text:this.title,
	...
	this.imageContainer = factory.create({t:"scene",parent:this.container, x:0, y:0,w:1255, a:1, h:this.container.h});
  ...
  this.imageObjs = [];
	...
	for( var i = 0; i < this.imageUrls.length; i++) {
	    ...
	    picture = factory.create({t:"image",url:url, parent:this.imageContainer, x:this.xPos, a:1, y:this.picY});
	...
	if( this.line == null) {
		...
		this.line	= factory.create({t:"image9",url:urlLine,a:1, x:0,y:picture.y+(this.picHeight*this.focusRatio)+10,h:2,w:this.container.w,parent:this.container});  
