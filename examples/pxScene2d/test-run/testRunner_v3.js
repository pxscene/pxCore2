...
var runTests = function( i) {  
  ...
  if( i < numUrls) {
      ...
      console.log("Creating new scene for test "+testName);
      ...
      var test  = scene.create({t:'scene',parent:root,url:url+manualString,x:0, y:0, w:scene.w, h:scene.h});
          ...
          testScene.ready.then(function() {
            ...
            timeouts[myIndex] = setTimeout( function() {
                  ...
                  console.log(">>>>>>>>>>>CANCELLING TESTS for "+testUrls[myIndex].url+" due to timeout!<<<<<<<<<")
                  runnerResults[testUrls[myIndex].title+".HANGING"]= "FAILURE";
                  testScene = null;
