...
pxTextLine::pxTextLine()
        ...
        , translateY(0)
{
    this->textFx.shadow.enabled = false;
    this->textFx.highlight.enabled = false;
}
...
bool pxTextLine::hasFx() const
{
    return textFx.shadow.enabled || textFx.highlight.enabled;
}
...
void pxTextCanvas::renderTextLine(const pxTextLine& textLine)
{
    ...
    rtLogDebug("pxTextCanva::renderTextLine; textline: '%s', current canvas: '%s' (w x h): %04.0f x %04.0f, shadow: %s, highlight: %s"
            ...
            , mw
            , mh
            , textLine.textFx.shadow.enabled ? "YES" : "NO"
            , textLine.textFx.highlight.enabled ? "YES" : "NO"
            );
    ...
    if( getFontResource() != nullptr)
    {
        ...
        if (textLine.hasFx()) quads.setFx(&textLine.textFx);
...
rtError pxTextCanvas::fillText(rtString text, int32_t x, int32_t y)
{
    ...
    // Shadow
    textLine.textFx.shadow.enabled       = false;  // default
    textLine.textFx.highlight.enabled = false;  // default

    if(shadow()) // creating the shadow effect config
    {
        memcpy(textLine.textFx.shadow.color, mShadowColor, sizeof(textLine.textFx.shadow.color));
        textLine.textFx.shadow.enabled = true;
        textLine.textFx.shadow.offsetX = mShadowOffsetX;
        textLine.textFx.shadow.offsetY = mShadowOffsetY;
        textLine.textFx.shadow.blur = mShadowBlur;
        textLine.textFx.shadow.width = getOnscreenWidth()  + textLine.textFx.shadow.offsetX;
        textLine.textFx.shadow.height = getOnscreenHeight() + textLine.textFx.shadow.offsetY;
    }

    if(mHighlight)// creating the hightlight effect config
    {
        memcpy(textLine.textFx.highlight.color, mHighlightColor, sizeof(textLine.textFx.highlight.color));
        textLine.textFx.highlight.enabled = true;
        textLine.textFx.highlight.width = getOnscreenWidth(); // block geometry
        textLine.textFx.highlight.height = getOnscreenHeight(); // block geometry
