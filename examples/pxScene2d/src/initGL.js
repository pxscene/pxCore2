...
var loadUrl = function(url, _beginDrawing, _endDrawing, _view) {
  ...
  const makeRequire = pathToParent => {
    return function (moduleName) {
      ...
      var filename = "";
      try {
        filename = (isLocalFile)?_module._resolveFilename(moduleName, {paths:[parentDir].concat(_module._nodeModulePaths(parentDir)),id:pathToParent,filename:pathToParent}):moduleName;
      }
      catch(e) {
        if (parentDir.indexOf("http") == 0) {
          isLocalFile = false;
          filename = parentDir + "/" + moduleName;
        }
      }
