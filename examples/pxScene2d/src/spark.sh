...
valgrind --vgdb=yes --tool=memcheck --gen-suppressions=all --suppressions=$SUPPRESSIONS  --log-file=$VALGRINDLOGS --leak-check=yes --show-reachable=yes --num-callers=20 --track-fds=yes ./Spark $1 $2 $3 $4 $5 $6 $7
fi
else
./Spark $1 $2 $3 $4 $5 $6 $7
fi
