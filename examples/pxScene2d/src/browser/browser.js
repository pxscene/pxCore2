...
var defaultHomeUrl = "https://www.pxscene.org/examples/px-reference/text/sample.md";
...
{  
  ...
  function canonicalizeUrl(u) {
    var Url = imports.Url
    urlParts = Url.parse(u,true)
    
    var protocol = urlParts.protocol
    if (protocol == 'http:' || protocol == 'https:') {
      delete urlParts.search
      if (urlParts.pathname.startsWith('/')) {
        var p = urlParts.pathname.split('/')
        var np = []
  ...
  var urlSucceededColor = 0x2E8B57FF
  var urlFailedColor    = 0xCD5C5CFF
  ...
  var bgSVG = (version.startsWith('edge_')||version.startsWith('dev'))?'/images/status_bg_edge.svg':'/images/status_bg.svg'
  ...
  var inputBox = new imports.EditBox( { parent: browser, url: base+'/images/input2.png', x: 70+10, y: 10, w: 800-70-32, h: 35, pts: 24 });
  ...
  var starRes   = scene.create({t:"imageResource", url: base+'/images/star-empty.svg', w:28, h:28});
  var star = scene.create({t:'image', parent:starContainer,h:28, w:28, resource: starRes, a:0.50, interactive:false})
  var starFillRes   = scene.create({t:"imageResource", url: base+'/images/star-filled.svg', w:28, h:28});
  var starFill = scene.create({t:'image', parent:starContainer,h:28, w:28, resource: starFillRes, a:0.5, interactive:false})
  var homeRes   = scene.create({t:"imageResource", url: base+'/images/home-solid.svg', w:28, h:28});
  var home = scene.create({t:'image', parent:browser,h:28, w:28, resource: homeRes, a:/*0.80*/0, interactive:false})
  var spinner   = scene.create({t:'image',  parent: browser, url: base+'/images/spinningball2.png',  y:-80, cx: 50, cy: 50, sx: 0.3, sy: 0.3,a:0.0,interactive:false});
  var backButtonRes = scene.create({ t: 'imageResource', url: base+'/images/arrow-circle-left-solid.svg', w: 32, h: 32 });
  var foreButtonRes = scene.create({ t: 'imageResource', url: base+'/images/arrow-circle-right-solid.svg', w: 32, h: 32 });
  var menuButtonRes = scene.create({ t: 'imageResource', url: base+'/images/bars-solid.svg', w: 24, h: 24 });
  ...
  var menu = scene.create({t:'image9',parent:menuButton,url:base+'/images/menu.png',insetLeft:10,insetRight:10,
      ...
      content.ready.then(
        ...
        function(gen) {
          return function(o) {
            ...
            if (gen == currentGen) {
              if (!keepHistory) {
                if (originalUrl != '' && currUrl != '' && currUrl != originalUrl) {
