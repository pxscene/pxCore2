...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function EditBox(params) {
        ...
        var    prompt_color =    "prompt_color" in params ? params.prompt_color    : "#f00"; //"#869CB2";    // gray ?
        var selection_color = "selection_color" in params ? params.selection_color : "#FCF2A488"; // yellow
        var      text_color =      "text_color" in params ? params.text_color      : "#303030";   // black
        ...
        {
          //console.log("DROPPED >>> onDragEnter" )
        });

        container.on("onDragLeave", function (e)
        {
          //console.log("DROPPED >>> onDragLeave" )
        });

        container.on("onDragDrop", function (e)
        {
          //console.log("DROPPED >>> textInput")

          if(e.type == scene.dragType.TEXT)
          {
          if(textInput.focus)
          {
            removeSelection(); // if any

            var org_pos = cursor_pos;
            textInput.text = textInput.text.slice(0, cursor_pos) + e.dropped + textInput.text.slice(cursor_pos); // INSERT

            if(org_pos == 0)
            {
              // move cursor to END of replacement text
              cursor_pos = textInput.text.length;
              console.log("updateCursor() EMPTY p: " + textInput.text.length)
              updateCursor(textInput.text.length);
            }
            else
            {
              // move cursor AFTER inserted text
              cursor_pos = org_pos + textInput.text.length;
              console.log("updateCursor() AFTER p: " + textInput.text.length)
              updateCursor(org_pos + e.dropped.length);
            }
          }
          else
          {
            textInput.text = e.dropped; // REPLACE
