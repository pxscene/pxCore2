...
public:
               ...
               int count, void *user)
  {
    ...
    glUniform1f(mAlphaLoc, alpha);
    glUniform4fv(mShadowColorLoc, 1, shdw->color);
...
void pxContext::drawTexturedQuads(int numQuads, const void *verts, const void* uvs,
                          pxTextureRef t, float* color, void *user /* = nullptr */)
{
#ifdef DEBUG_SKIP_IMAGE
#warning "DEBUG_SKIP_IMAGE enabled ... Skipping "
  return;
...
pxContextFramebufferRef pxContext::applyBlurSettings(pxContextFramebufferRef src, shadowFx_t *shdw, filterXYR *filters, size_t count)
{
  ...
  do
  {
    ...
    context.drawEffect( 0,0, w,h, fbo_src->getTexture(), gATextureBlurShader, (void *) shdw);
                                  ...
                                  pxTextureRef t, textFx_t *textFx)
{
  ...
  //
  if(high && high->enabled)
  {
    // Draw HIGHLIGHT directly on FrameBuffer ... at back
    //

    // float padding = (high->paddingLeft + high->paddingRight);

    context.pushState();

    // TODO: '0.575' is an Egregious MAGIC NUMBER
    pxMatrix4f m;
    m.translate(high->paddingLeft, high->offset + high->height * 0.575); // apply highlight offset
    context.setMatrix(m);

    rtLogInfo("drawTextEffects, highlight: w: %f, h: %f", high->width, high->height);
    context.drawRect(high->width - (high->paddingLeft + high->paddingRight), high->height * 0.2, 0, high->color, NULL); // TODO: '0.2' is an Egregious MAGIC NUMBER
  ...
  if(shdw && shdw->enabled)
  {
    ...
    pxMatrix4f m;
    m.translate(shdw->offsetX, shdw->offsetY); // apply shadow offset
    context.setMatrix(m);
    ...
    for(size_t i =0; i<len; i++)
    {
      ...
      output = applyBlurSettings(input, shdw, blur_settings, count);
...
{
  ::drawEffect(x, y, w, h, t, (aLinearBlurShaderProgram *)shader, user);
};
