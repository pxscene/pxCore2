                              ...
                              int32_t count, rtObjectRef& animateObj)
{
  ...
  if(duration.getType() == RT_stringType)
  {
    ...
    if(duration.getString(str) == RT_OK)
    {
      d = getDurationSeconds(str);
    }
  }
  else
  {
    duration.getDouble(d);
  }

  rtObjectRef promise = new rtPromise();
  animateObj = new pxAnimate(props, interp, (pxConstantsAnimation::animationOptions)options, d, count, promise, this);
  if (mIsDisposed)
  {
    rtLogWarn("animation is performed on disposed object !!!!");
    rtValue nullValue;
  ...
  if (keys)
  {
    ...
    {
      rtString key = keys.get<rtString>(i);
      animateToInternal(key, props.get<float>(key), d, ((pxConstantsAnimation*)CONSTANTS.animationConstants.getPtr())->getInterpFunc(interp), (pxConstantsAnimation::animationOptions)options, count,(i==0)?promise:rtObjectRef(),animateObj);
    }
  }
