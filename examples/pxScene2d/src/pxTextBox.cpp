...
void pxTextBox::renderTextWithWordWrap(const char *text, float sx, float sy, float tempX, uint32_t size, float* color, bool render)
{
  ...
  if( mAlignHorizontal == H_LEFT && mTruncation != NONE ) 
  {
    if( mXStopPos != 0) 
    {
...
//      setLineMeasurements(true, tempX, tempY);
    ...
    // Read char by char to determine full line of text before rendering
		while((charToMeasure = u8_nextchar((char*)text, &i)) != 0) 
		{
			...
			tempChar[1] = 0;
			((pxFont*)mFont.getPtr())->measureTextChar(charToMeasure, size,  sx, sy, charW, charH);
        ...
        {  // If not lastLine
					...
					if( isWordBoundary(tempStr[n])) 
          {
						...
						if( !isSpaceChar(tempChar[0]) || (isSpaceChar(tempChar[0]) && accString.length() != 0)) 
            {
					...
					((pxFont*)mFont.getPtr())->measureTextInternal(accString.cString(), size, sx, sy, charW, charH);
          ...
          {
            lastLine = true;
            if(mXStopPos != 0 && mAlignHorizontal == H_LEFT) 
            {
      ...
      if( mTruncation == NONE && !mWordWrap ) {
        ...
        if( !lastLine && mXStopPos != 0 && mAlignHorizontal == H_LEFT && mTruncation != NONE && mXStopPos > mXStartPos
            && tempX > mw) {
  ...
  if( !render) {
      float metricHeight;
      ((pxFont*)mFont.getPtr())->getHeight(mPixelSize,metricHeight);
      ...
      else if( mAlignVertical == V_CENTER)
      {
        if(!mWordWrap ) 
        {
...
void pxTextBox::renderOneLine(const char * tempStr, float tempX, float tempY, float sx, float sy, uint32_t size, float* color, float lineWidth, bool render )
{
  ...
  float xPos = tempX; 
  ((pxFont*)mFont.getPtr())->measureTextInternal(tempStr, size, sx, sy, charW, charH);
  ...
  if( !clip() && mTruncation == NONE) 
  {
    ...
    if( !mWordWrap)  
    {
      if( mAlignHorizontal == H_CENTER ) 
      { 
        xPos = tempX;
        noClipX = (lineWidth/2) - charW/2;
      }
      else if( mAlignHorizontal == H_RIGHT) 
      {
        xPos = tempX;
        noClipX = mw-charW;
      }
  ...
  // Now, render the text
  if( render) {((pxFont*)mFont.getPtr())->renderText(tempStr, size, xPos, tempY, sx, sy, color,lineWidth);} 
...
void pxTextBox::setLineMeasurements(bool firstLine, float xPos, float yPos) 
{
  ...
  ((pxFont*)mFont.getPtr())->getMetrics(mPixelSize, height, ascent, descent, naturalLeading);
  if(!firstLine) {
...
void pxTextBox::renderTextNoWordWrap(float sx, float sy, float tempX, bool render)
{
  ...
  ((pxFont*)mFont.getPtr())->measureTextInternal(mText, mPixelSize, sx, sy, charW, charH);
  ...
  else 
  {
    ...
    if( mAlignVertical == V_BOTTOM || mAlignVertical == V_CENTER) 
    {
      if( mAlignVertical == V_BOTTOM ) 
      {
                                          ...
                                          float sx, float sy, uint32_t pixelSize, float* color, bool render) 
{
  ...
  {
		length -= ELLIPSIS_LEN;
		((pxFont*)mFont.getPtr())->measureTextInternal(ELLIPSIS_STR, pixelSize, sx, sy, ellipsisW, charH);
  ...
  { 
		...
		charW = 0;
		charH = 0;
		((pxFont*)mFont.getPtr())->measureTextInternal(tempStr, pixelSize, sx, sy, charW, charH);
		if( (tempX + charW + ellipsisW) <= lineWidth) 
    {
			...
			if( mTruncation == TRUNCATE) 
      {
        ...
        if( render) {
          ((pxFont*)mFont.getPtr())->renderText(tempStr, pixelSize, xPos, tempY, 1.0, 1.0, color,lineWidth);
        } 
				if( mEllipsis) 
        {
					//printf("rendering truncated text with ellipsis\n");
					if( render) {
            ((pxFont*)mFont.getPtr())->renderText(ELLIPSIS_STR, pixelSize, xPos+charW, tempY, 1.0, 1.0, color,lineWidth);
          } 
...
rtError pxTextBox::getFontMetrics(rtObjectRef& o) {
  ...
  ((pxFont*)mFont.getPtr())->getFontMetrics(mPixelSize, o);
