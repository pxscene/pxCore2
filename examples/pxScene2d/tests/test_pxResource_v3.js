...
var tests = {
  ...
  test4: function() {
    return new Promise(function(resolve, reject) {
      var svgRes = scene.create({t:"imageResource",parent:root, url:"https://px-apps.sys.comcast.net/pxscene-samples/images/SVG_logo.svg"});
      var results = [];
      var tmpTimeout = setTimeout(function(){
        resolve(assert(false,">>>>> TIMEOUT ON test4 svgRes: waiting for promise"));
      }, 300000);
      svgRes.ready.then(function()  {
        clearTimeout(tmpTimeout);
        console.log("test4: svgRes ready");
        var res = svgRes.resource;
        var loadStatus = res.loadStatus;
        // check value 
        results.push(assert(svgRes.url=="https://px-apps.sys.comcast.net/pxscene-samples/images/SVG_logo.svg","url is not correct"));
        resolve(results);
      }, function(o){
        clearTimeout(tmpTimeout);
        console.log("test4: svgRes rejection");
        var res = svgRes.resource;
        var loadStatus = res.loadStatus;
        results.push(assert(loadStatus["statusCode"]==0,"status code is not correct; code is "+loadStatus["statusCode"]));
        results.push(assert(false,"svgRes promise rejection was unexpected!"));
        reject(results);
      });
    });
  }, 
  ...
  test5: function() {
    return new Promise(function(resolve, reject) {
      var svgImg = scene.create({t:"image",parent:root, url:"https://px-apps.sys.comcast.net/pxscene-samples/images/SVG_logo.svg"});
      var results = [];
      var tmpTimeout = setTimeout(function(){
        resolve(assert(false,">>>>> TIMEOUT ON test5 svgImg: waiting for promise"));
      }, 300000);
      svgImg.ready.then(function()  {
        clearTimeout(tmpTimeout);
        console.log("test5: svgImg ready");
        var res = svgImg.resource;
        var loadStatus = res.loadStatus;
        // check value 
        results.push(assert(svgImg.url=="https://px-apps.sys.comcast.net/pxscene-samples/images/SVG_logo.svg","url is not correct"));
        resolve(results);
      }, function(o){
        clearTimeout(tmpTimeout);
        console.log("test5: svgImg rejection");
        var res = svgImg.resource;
        var loadStatus = res.loadStatus;
        results.push(assert(loadStatus["statusCode"]==0,"status code is not correct; code is "+loadStatus["statusCode"]));
        results.push(assert(false,"svgImg promise rejection was unexpected!"));
        reject(results);
      });
    });
  }
