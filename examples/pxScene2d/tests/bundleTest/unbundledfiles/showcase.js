...
}).then( function importsAreReady(imports)
{
  ...
  function createScene(w, h)
  {
    ...
    let assets = [container.ready, placeholderRes.ready, roundedRectRes.ready ];

    return Promise.all(assets).then( (o) =>
    {
      container.on('onKeyDown', onKeyDown);

      scrollable = new Scrollable(scene, container, {rowScrollHeight: tileH});

      return Promise.all([scrollable.ready]).then( (o) =>
      {
...
//    console.log("doFocus()  >>> " + o.info.title);
  ...
  {
    if ( keys.is_CTRL(e.flags) || keys.is_CTRL_ALT(e.flags) )  
    {
      if( selected.focus == false &&
          ((e.keyCode == keys.L)      || // LAST
    ...
    if(selected.focus == true)
    {
      ...
      if( dx != 0 || dy !=0 ) // Any ARROW key change ?
      {
        ...
        if ( sub2ind(pos_x, pos_y) > maxIndex )  // previous row ? ... landed on an empty spot on this row.
        {
          if(Math.abs(dx))
          {
            if(dx > 0)  pos_x = 0;
  ...
  function showScene(target)
  {
    ...
    console.log("Launching App: " + info.attributes.url + " ...");
